...
async fn builtin_identity() {
    ...
    assert_match(&text, &entities, builtins::TAX_ID.to_ref(), "65929970489");
    assert_match(&text, &entities, builtins::TAX_ID.to_ref(), "DE129273398");
    assert_match(
        &text,
        &entities,
...
async fn builtin_health() {
    ...
    assert_match(&text, &entities, builtins::MEDICAL_ID.to_ref(), "381789045");
    assert_match(&text, &entities, builtins::MEDICAL_ID.to_ref(), "123456601");
    assert_match(
        &text,
        &entities,
...
async fn builtin_contact() {
    ...
    assert_match(&text, &entities, builtins::POSTAL_CODE.to_ref(), "10117");
