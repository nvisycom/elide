//! End-to-end through the **public facade API**: a PNG's EXIF metadata is
...
async fn png_gps_is_stripped_through_the_facade() {
    ...
    assert!(has_gps_png(&original), "fixture should carry GPS");
    ...
    let handle = registry
        .decode(original.clone(), "png")
        .await
        .expect("decode png");
    let mut documents = [Document::new("photo.png", handle)];
