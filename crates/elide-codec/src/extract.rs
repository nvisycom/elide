...
//!
//! Many formats, markup (HTML, XML) and rich documents (DOCX), differ in
//! their *parser* and *serializer* but share the same redactable shape: a
//! sequence of text-valued units, each carrying an encoder-private address,
//! redacted as text and spliced back into the native container. This module
//! is that neutral core:
//!
...
/// that item's decoded value to replace. The common currency of the handler's
/// own resolution step and [`Encoder::locate_source`], which both answer
...
#[derive(Debug)]
pub struct ExtractHandler<E: Encoder> {
    format_id: FormatId,
    encoder: E,
