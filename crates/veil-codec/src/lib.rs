...
mod tests {
    use veil_core::modality::DataWriter;
    use veil_core::modality::text::{Text, TextLocation, TextReplacement};
    ...
    async fn registry_decodes_txt_by_extension() {
        ...
        let handle = reg
            ...
            .await
            .expect("txt decoded");
        assert_eq!(handle.format_id().as_str(), "veil.text.txt");
    ...
    async fn untyped_into_wrong_modality_returns_self() {
        ...
        assert_eq!(typed.format_id().as_str(), "veil.text.txt");
    }
