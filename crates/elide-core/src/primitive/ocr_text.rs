//! [`OcrText`]: OCR output addressable as text, with per-word regions.
//!
//! An OCR pass is what makes an image *recognizable*: a recognizer reads
//! its [`text`](OcrText::text) like any other string, finds a match at a
//! byte range, and [`resolve`](OcrText::resolve)s that range back to the
//! [`ImageLocation`] of the words it covers — via the per-word bounding
//! boxes the OCR carries. The image counterpart of the audio transcription.
...
use super::{BoundingBox, Confidence, Point};
use crate::modality::image::ImageLocation;

/// Separator inserted between blocks when building the flat OCR text, so
/// adjacent blocks don't run their words together.
const BLOCK_SEPARATOR: &str = "\n";

/// OCR output for one image, addressable as flat text.
///
/// An ordered set of [`OcrBlock`]s (the recognized text regions). The flat
...
/// One word within an [`OcrBlock`], with its own bounding box.
#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct OcrWord {
...
impl OcrText {
    /// Build OCR text from blocks, computing the flat text.
    #[must_use]
    ...
    /// The flat OCR text a recognizer inspects: the blocks' text joined.
    #[must_use]
    ...
    /// Blocks in reading order.
    #[must_use]
    pub fn blocks(&self) -> &[OcrBlock] {
        &self.blocks
    }

    /// Whether the OCR recognized no blocks.
    #[must_use]
    ...
    fn block_offsets(&self) -> impl Iterator<Item = (usize, &OcrBlock)> {
        let mut offset = 0;
...
impl RegionUnion {
    ...
    fn add_words(&mut self, block: &OcrBlock, range: Range<usize>) {
...
mod tests {
    ...
    fn word(x: f64, y: f64, w: f64, h: f64, text: &str) -> OcrWord {
        OcrWord::new(loc(x, y, w, h), text)
    }

    /// "Call Alice" as one block of two boxed words.
    fn two_word_block() -> OcrBlock {
        OcrBlock::new(loc(0.0, 0.0, 100.0, 20.0), "Call Alice").with_words(vec![
    ...
    fn text_is_blocks_joined() {
        let t = OcrText::new(vec![
            OcrBlock::new(loc(0.0, 0.0, 10.0, 10.0), "hello"),
            OcrBlock::new(loc(0.0, 20.0, 10.0, 10.0), "world"),
        ]);
    ...
    fn resolve_maps_a_word_range_to_its_box() {
        let t = OcrText::new(vec![two_word_block()]);
    ...
    fn resolve_falls_back_to_block_region_without_words() {
        let t = OcrText::new(vec![OcrBlock::new(loc(5.0, 5.0, 30.0, 10.0), "no word boxes")]);
