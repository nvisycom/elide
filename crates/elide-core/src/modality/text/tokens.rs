//! [`Token`] and the [`Tokens`] collection: text's enrichment artifact.
//!
...
//!
//! The shape is intentionally minimal. POS tags, morphology, and dependency
//! trees are not part of the surface; they get added as fields when a downstream
//! consumer needs them. `text == lemma`, `is_stop == false`, and
//! `is_punct == false` are the defaults for a tokenizer-only engine.
...
impl Token {
    /// Construct a token with the surface form mirrored into `lemma`. Use this
    /// from tokenizer-only producers.
...
impl Tokens {
    ...
    /// tokenizer (language-only engines).
    #[must_use]
    pub fn empty() -> Self {
        Self::default()
    }

    /// Construct from an owned vector. The caller is responsible for ensuring
    /// tokens are sorted by `offset.start`.
    #[must_use]
