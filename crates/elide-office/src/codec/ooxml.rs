//! The shared OOXML codec layer: helpers common to the docx and pptx
//! adapters over this crate's package engine.
//!
//! The two text-only OOXML formats decode to the same [`OoxmlPackage`] and
//! adapt to the codec's [`Encoder`] / [`Container`] contract the same way; this
//! module holds the pieces that would otherwise be copy-pasted across them:
//! extract element-text blocks addressed by `(part, span, OffsetMap)`,
//! surface embeddings and document-property parts as sub-parts, and re-pack
//! via [`rewrite_with_parts`](OoxmlPackage::rewrite_with_parts). The only
//! per-format differences, the engine format, the codec [`FormatId`], and the
//! label used in errors, are named by the [`OoxmlCodec`] seam; everything
...
use elide_codec::{Container, FormatId, LocalId, Part};
...
pub(crate) struct OoxmlEncoder<C: OoxmlCodec> {
    /// The original package bytes, retained so the engine can re-pack every
...
impl<C: OoxmlCodec> Encoder for OoxmlEncoder<C> {
    ...
    fn encode(&self, items: &[ExtractedItem<OoxmlAddress>]) -> Result<ContentData> {
        ...
        // part's XML. The engine validates and applies these fail-closed.
...
impl<C: OoxmlCodec> Container for OoxmlEncoder<C> {
    fn parts(&self) -> Vec<Part> {
        ...
        let props = self.doc_props.iter().map(|(path, bytes)| Part {
            ...
            bytes: bytes.clone(),
            hint: super::docprops_hint().to_owned(),
        });
...
pub(crate) fn decode_extract<C: OoxmlCodec>(content: ContentData) -> Result<OoxmlHandler<C>> {
    ...
    let pkg = OoxmlPackage::<C::Format>::open(&archive).map_err(super::office_error)?;
    // Cache the property parts' bytes so the `Container` can surface them as
    // metadata sub-parts without re-opening the package.
    let doc_props = super::props::read_doc_props(|path| pkg.part_bytes(path));
