//! XLSX loader: open the workbook via this crate's engine, extract its
//! cells, and hand them to the [`XlsxHandler`], retaining the archive for
//! re-pack.

use elide_codec::Loader;
use elide_codec::content::ContentData;
use elide_core::Result;
use elide_core::modality::tabular::Tabular;

use super::xlsx_handler::{XlsxCell, XlsxHandler};
use crate::xlsx::Xlsx;

/// Loader for XLSX files. Produces one [`XlsxHandler`] per input.
#[derive(Debug)]
pub(crate) struct XlsxLoader;

#[async_trait::async_trait]
impl Loader for XlsxLoader {
    type Handler = XlsxHandler;
    type Modality = Tabular;

    async fn decode(&self, content: ContentData) -> Result<XlsxHandler> {
        let archive = content.to_bytes();
        let workbook = Xlsx::open(&archive).map_err(super::office_error)?;
        let cells = workbook
            .extract()
            .map_err(super::office_error)?
            .into_iter()
            .map(|cell| XlsxCell {
                sheet: cell.sheet.as_str().to_owned(),
                row: cell.row,
                column: cell.column,
                text: cell.text.as_str().to_owned(),
            })
            .collect();
        let text_parts = workbook.text_parts();
        let doc_props = super::props::read_doc_props(|path| workbook.part_bytes(path));
        Ok(XlsxHandler::new(archive, cells, text_parts, doc_props))
    }
}
