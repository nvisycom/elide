...
impl PartKind {
    ...
    pub fn of(part: &PartPath) -> Self {
        ...
        if path == "word/glossary/document.xml"
            || part.numbered("word/glossary/header", ".xml")
            || part.numbered("word/glossary/footer", ".xml")
        {
            return Self::Glossary;
        }
        if part.numbered("word/header", ".xml") {
            return Self::Header;
        }
        if part.numbered("word/footer", ".xml") {
            return Self::Footer;
        }
        ...
        if part.numbered("word/charts/chart", ".xml") {
            return Self::Chart;
        }
        if part.numbered("word/diagrams/data", ".xml") {
            return Self::Diagram;
        }
        ...
        if let Some(kind) = part.embedding_under("word") {
...
mod tests {
    ...
    #[test]
    fn relationships_parts_are_classified_as_relationship_targets() {
        // The package root rels and a part's sidecar rels are both relationships.
        // They hold their redactable PII in attribute targets, not element text.
        for path in ["_rels/.rels", "word/_rels/document.xml.rels"] {
            assert_eq!(kind(path), PartKind::Relationships, "{path}");
            assert_eq!(kind(path).role(), PartRole::RelationshipTargets, "{path}");
        }
    }
